...
});class CreditNoteTowRateExport implements FromView, WithEvents
{
    public  $creditNote;
    public $invoice;
    ...
    public function view(): view
    {
      ...
      return view("admin::pages.credit_note.credit-note-tow-rate-excel", [
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                //head list
                $event->sheet->styleCells("B17:J17", [
                    'font' => array(
                        'name' => 'Khmer OS',
                        'size' => "7",
                        'bold' => true
                ...
                $event->sheet->styleCells("B18:J18", [
                    'font' => array(
                        'name' => 'Calibri',
                        'size' => "8",
                        'bold' => true
                ...
                $event->sheet->getDelegate()->getStyle('B17:J18')
                    ->getFill()
                ...
                $event->sheet->styleCells("B19:J19", [
                    'font' => array(
                        'name' => 'Calibri',
                        'size' => "8",
                        'bold' => true
                ...
                for ($i = 0; $i < count($columns); $i++) {
                    if ($i == 0) {
                        ...
                        $event->sheet->getDelegate()->getColumnDimension($columns[$i])->setWidth(9);
                    } elseif ($i == 7) {
                ...
                for ($i = 20 + count($this->invoice_detail); $i <= 23 + count($this->invoice_detail); $i++) {
                    $event->sheet->styleCells("B$i:J$i", [
                        'font' => array(
                            'name' => 'Calibri',
                            'size' => "8",
                        ),
                ...
                $event->sheet->styleCells("H$row_re:J$row_re", [
                    'font' => array(
                        'name' => 'Calibri',
                        'size' => "9",
                        'bold' => true
                    ...
                    for ($j = 0; $j < count($columns); $j++) {
                        ...
                        if ($j == 8) {
                            ...
                            if ($this->invoice->charge_type == 'day') {
                                ...
                                if($this->invoice){
                                    if($this->invoice->InvoiceDetail[$i-20]->rate_first && $this->invoice->InvoiceDetail[$i-20]->rate_second){
                                        $event->sheet->getDelegate()->getCell($columns[$j] . $i)->setValue("=ROUND(E$i*G$i*H$i*I$i/12*$charge_number,2)");
                                    }
                                    elseif($this->invoice->InvoiceDetail[$i-20]->rate_first && !$this->invoice->InvoiceDetail[$i-20]->rate_second){
                                        $event->sheet->getDelegate()->getCell($columns[$j] . $i)->setValue("=ROUND(E$i*G$i*H$i/12*$charge_number,2)");
                                    }
                                    elseif(!$this->invoice->InvoiceDetail[$i-20]->rate_first && $this->invoice->InvoiceDetail[$i-20]->rate_second){
                                        $event->sheet->getDelegate()->getCell($columns[$j] . $i)->setValue("=ROUND(E$i*G$i*I$i/12*$charge_number,2)");
                                    }
                                    else{
                                ...
                                if($this->invoice){
                                    if($this->invoice->InvoiceDetail[$i-20]->rate_first && $this->invoice->InvoiceDetail[$i-20]->rate_second){
                                        $event->sheet->getDelegate()->getCell($columns[$j] . $i)->setValue("=ROUND(E$i*G$i*H$i*I$i/$charge_number,2)");
                                    }
                                    elseif($this->invoice->InvoiceDetail[$i-20]->rate_first && !$this->invoice->InvoiceDetail[$i-20]->rate_second){
                                        $event->sheet->getDelegate()->getCell($columns[$j] . $i)->setValue("=ROUND(E$i*G$i*H$i/$charge_number,2)");
                                    }
                                    elseif(!$this->invoice->InvoiceDetail[$i-20]->rate_first && $this->invoice->InvoiceDetail[$i-20]->rate_second){
                                        $event->sheet->getDelegate()->getCell($columns[$j] . $i)->setValue("=ROUND(E$i*G$i*I$i/$charge_number,2)");
                                    }
                                    else{
                ...
                $event->sheet->getDelegate()->getCell("I$row_total")->setValue("=SUM(J20:J$end)");
                $event->sheet->getDelegate()->getStyle("I$row_total")->getNumberFormat()
                    ->setFormatCode('"$"* #,##0.00');

                $row_vat = 25 + $this->invoice_detail->count();
                $event->sheet->getDelegate()->getCell("I$row_vat")->setValue("=ROUND(I$row_total*10%,2)");
                $event->sheet->getDelegate()->getStyle("I$row_vat")->getNumberFormat()
                    ->setFormatCode('"$"* #,##0.00');

                $row_grand = 26 + $this->invoice_detail->count();
                $event->sheet->getDelegate()->getCell("I$row_grand")->setValue("=ROUND(I$row_total+I$row_vat,2)");
                $event->sheet->getDelegate()->getStyle("I$row_grand")->getNumberFormat()
                    ->setFormatCode('"$"* #,##0.00');

                //khmer
                $rate = $this->rate->rate;
                $event->sheet->getDelegate()->getCell("J$row_grand")->setValue("=ROUND(I$row_grand*$rate,2)");
                $event->sheet->getDelegate()->getStyle("J$row_grand")->getNumberFormat()
                    ->setFormatCode('"R"* #,##0');

                $event->sheet->getDelegate()->getCell("J$row_total")->setValue("=ROUND(J$row_grand/1.1,2)");
                $event->sheet->getDelegate()->getStyle("J$row_total")->getNumberFormat()
                    ->setFormatCode('"R"* #,##0');

                $event->sheet->getDelegate()->getCell("J$row_vat")->setValue("=ROUND(J$row_grand-J$row_total,2)");
                $event->sheet->getDelegate()->getStyle("J$row_vat")->getNumberFormat()
                    ->setFormatCode('"R"* #,##0');
                ...
                $event->sheet->getStyle("B10:J$end")->applyFromArray([
                    'borders' => [
                        'allBorders' => [
                ...
                //head in table list
                $event->sheet->getStyle("B17:J18")->applyFromArray([
                    'borders' => [
                        'outline' => [
                            'borderStyle' => \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_MEDIUM,
                        ],
                ...
                $end_r = $start_r + 2;
                $event->sheet->getStyle("B$start_r:J$end_r")->applyFromArray([
                    'borders' => [
                        'allBorders' => [
                            'borderStyle' => \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_MEDIUM,
                        ],
                ...
                $event->sheet->getStyle("B$start_r:F$end_r")->applyFromArray([
                    'borders' => [
                        'horizontal' => [
                            'borderStyle' => \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_NONE,
                        ],
