...
class SubmarineInvoiceExport implements FromView, WithEvents
{
    use Exportable;
    public $data;
    public $totalPrice;
    ...
    {
        return view("admin::pages.report.summary_invoice.submarine.excel", [
            'data' => $this->data,
        ...
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                for ($i = 3; $i <= count($this->data) + 2; $i++) {
                    $event->sheet->getDelegate()->getStyle("G$i")->getNumberFormat()->setFormatCode('"$"* #,##0.00;"$"* -#,##0.00');
                ...
                $event->sheet->getDelegate()->getStyle("G$i")->getNumberFormat()->setFormatCode('"$"* #,##0.00;[Red]"$"* -#,##0.00');
