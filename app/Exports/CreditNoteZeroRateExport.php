...
class CreditNoteZeroRateExport implements FromView, WithEvents
{
    public  $creditNote;
    public $invoice;
    ...
    public function view(): view
    {
      ...
      return view("admin::pages.credit_note.credit-note-zero-rate-excel", [
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $event->sheet->getDelegate()->getColumnDimension('I')->setVisible(false);
                ...
                //head list
                $event->sheet->styleCells("B17:H17", [
                    'font' => array(
                        'name' => 'Khmer OS',
                        'size' => "7",
                        'bold' => true
                ...
                $event->sheet->styleCells("B18:H18", [
                    'font' => array(
                        'name' => 'Calibri',
                        'size' => "8",
                        'bold' => true
                ...
                $event->sheet->getDelegate()->getStyle('B17:H18')
                    ->getFill()
                ...
                for ($i = 0; $i < count($columns); $i++) {
                    if ($i == 0) {
                        ...
                        $event->sheet->getDelegate()->getColumnDimension($columns[$i])->setWidth(17);
                    }
                }

                for ($i = 20 + count($this->invoice_detail); $i <= 23 + count($this->invoice_detail); $i++) {
                    $event->sheet->styleCells("B$i:H$i", [
                        'font' => array(
                            'name' => 'Calibri',
                            'size' => "8",
                        ),
                ...
                $event->sheet->styleCells("E$row_re:F$row_re", [
                    'font' => array(
                        'name' => 'Khmer OS',
                        'size' => "6",
                        'bold' => true
                ...
                $event->sheet->styleCells("G$row_re:H$row_re", [
                    'font' => array(
                        'name' => 'Calibri',
                        'size' => "9",
                        'bold' => true
                ...
                $event->sheet->getDelegate()->getCell("G$row_total")->setValue("=SUM(H20:H$end)");
                $event->sheet->getDelegate()->getStyle("G$row_total")->getNumberFormat()
                  ->setFormatCode('"$"* #,##0.00');
        
                $row_vat = 25 + $this->invoice_detail->count();
                $event->sheet->getDelegate()->getCell("G$row_vat")->setValue("=ROUND(G$row_total*10%,2)");
                $event->sheet->getDelegate()->getStyle("G$row_vat")->getNumberFormat()
                  ->setFormatCode('"$"* #,##0.00');
        
                $row_grand = 26 + $this->invoice_detail->count();
                $event->sheet->getDelegate()->getCell("G$row_grand")->setValue("=ROUND(G$row_total+G$row_vat,2)");
                $event->sheet->getDelegate()->getStyle("G$row_grand")->getNumberFormat()
                  ->setFormatCode('"$"* #,##0.00');
        
                //khmer
                $rate = $this->rate->rate;
                $event->sheet->getDelegate()->getCell("H$row_grand")->setValue("=ROUND(G$row_grand*$rate,2)");
                $event->sheet->getDelegate()->getStyle("H$row_grand")->getNumberFormat()
                  ->setFormatCode('"R"* #,##0');
        
                $event->sheet->getDelegate()->getCell("H$row_total")->setValue("=ROUND(H$row_grand/1.1,2)");
                $event->sheet->getDelegate()->getStyle("H$row_total")->getNumberFormat()
                  ->setFormatCode('"R"* #,##0');
        
                $event->sheet->getDelegate()->getCell("H$row_vat")->setValue("=ROUND(H$row_grand-H$row_total,2)");
                ...
                $event->sheet->getStyle("B10:H$end")->applyFromArray([
                    'borders' => [
                        'allBorders' => [
                ...
                //head in table list
                $event->sheet->getStyle("B17:H18")->applyFromArray([
                    'borders' => [
                        'outline' => [
                            'borderStyle' => \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_MEDIUM,
                        ],
                ...
                $end_r = $start_r + 2;
                $event->sheet->getStyle("B$start_r:H$end_r")->applyFromArray([
                    'borders' => [
                        'allBorders' => [
                            'borderStyle' => \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_MEDIUM,
                        ],
                ...
                $event->sheet->getStyle("B$start_r:D$end_r")->applyFromArray([
                    'borders' => [
                        'horizontal' => [
                            'borderStyle' => \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_NONE,
                        ],
