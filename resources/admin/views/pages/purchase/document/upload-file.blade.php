    ...
    <script>
        Alpine.data('uploadFile', () => ({
            ...
            onSelectFile() {
                ...
                const pac_id = @json($purchase->id);
                const url =
                    `{{ route('admin-purchase-upload') }}?folder_id=${lastFolder?.id ?? ''}&pac_id=${pac_id}`;
                $onUploadFile(url, true,
                    'application/vnd.ms-excel,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet,application/pdf,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document',
                    (loading,
