<div class="file-manager-wrapper pt-5 pe-5 ps-5" x-data="fileManager">
    ...
    <div class="file-manager-body">
        ...
        <div class="file-side">
            @include('admin::pages.purchase.document.menu')
                                ...
                                <div class="file-item" x-bind:class="isSelected(file, 'selected')"
                                    ...
                                    <div class="img" style="background-color: white;">
                                        <template x-if="file?.extension == 'pdf'">
                                            <img src='{{ asset('images/file/pdf.png') }}' alt="" class="p-1">
                                        </template>
                                        <template x-if="file?.extension == 'xlsx' || file?.extension == 'xls'">
                                            <img src='{{ asset('images/file/excel.webp') }}' alt=""
                                                class="p-1">
                                        </template>
                                        <template x-if="file?.extension == 'doc'|| file?.extension =='docx'">
                                            <img src='{{ asset('images/file/word.png') }}' alt=""
                                                class="p-2">
                                        </template>
                                        <template x-if="file?.extension == 'ppt'|| file?.extension ==  'pptx'">
                                            <img src='{{ asset('images/file/powerpoint.png') }}' alt=""
                                                class="p-2">
                                        </template>
                                    </div>
                                    <div class="name">
    ...
    <template x-if="dialog.component.createFolderDialog">
        @include('admin::pages.purchase.document.create-folder')
    </template>
    <template x-if="dialog.component.renameFolderDialog">
        @include('admin::pages.purchase.document.rename-folder')
    </template>
    <template x-if="dialog.component.uploadFileDialog">
        @include('admin::pages.purchase.document.upload-file')
    </template>
    <template x-if="dialog.component.confirmDialog">
        @include('admin::pages.purchase.document.trash-dialog')
    </template>
        ...
        <div class="dialog">
            <div class="context-menu" :style="`top:${contentMenu.y}px;left:${contentMenu.x}px`">
                ...
                <template x-if="contentMenu.type === 'file'">
                    <div class="context-menu-item" @click="download(contentMenu?.data)"
                        @click.away="closeContextMenu">
                        <span><a download="" x-bind:href="pathDownloadImage" style="color:black;"> <span
                                    class="me-5">Download</span><span class="ms-3"><i
                                        data-feather="download"></i></span></a></span>
                    </div>
                </template>
    ...
    <script>
        Alpine.data('fileManager', () => ({
            ...
            getData(params = {}) {
                ...
                Axios
                    .get(`{{ route('admin-purchase-first', $purchase->id) }}`, {
                        params: params,
                    })
                    .then((data) => {
                        this.filePage += 1;
                        this.files = data.data.files?.data ? data.data.files.data : [];
                        this.folders = data.data.folders ? data.data.folders : [];
            ...
            getFile(params = {}, next) {
                ...
                Axios
                    .get(`{{ route('admin-purchase-files', $purchase->id) }}`, {
                        params: params,
                    })
                    .then((data) => {
            ...
            download(file) {
                this.pathDownloadImage = this.base_path + file.path;
            },
            showTooltip(el, file) {
