                                ...
                                <template x-if="!$store.create.options.data">
                                    <div class="form-row">
                                        ...
                                        <div class="pickup_invoice" style="width: 100%;"
                                            ...
                                            <input type="text" x-show="!dataForm.receipt_from || dataForm.receipt_from == 'deposit'" placeholder="Select ref invoice number ..." x-model="dataForm.invoice_number" readonly />
                                            <input type="text" x-show="dataForm.receipt_from == 'invoice'" placeholder="Select ref invoice number ..." x-model="dataForm.invoice_number" @click="selectInvoice()" readonly />
                                            <input type="text" x-show="dataForm.receipt_from == 'credit_note'" placeholder="Select ref credit note number ..." x-model="dataForm.invoice_number" @click="selectCreditNote()" readonly />
                                            ...
                                            <template x-if="dataForm.invoice_number">
                                                <button type="button" class="bgRed" @click="ResetPathInput()">Reset</button>
                                            </template>
                                        </div>
    ...
    Alpine.data('xCreate', () => ({
        ...
        selectInvoice() {
            ...
            SelectOption({
                ...
                onReady: (callback_data) => {
                    Axios({
                            url: `{{ route('admin-select-work-order-invoice') }}`,
                            method: 'GET'
                        })
                        .then(response => {
        ...
        selectCreditNote() {
            ...
            SelectOption({
                ...
                onReady: (callback_data) => {
                    Axios({
                            url: `{{ route('admin-select-work-order-credit-note') }}`,
                            method: 'GET'
                        })
                        .then(response => {
                            const data = response?.data?.data.map(item => {
        ...
        async PickUpInvoice() {
            ...
            dataTimeOut = setTimeout(async () => {
                try {
                    let pickUpInvoiceUrl = `/admin/work-order/receipt/pick-up-invoice/${this.dataForm?.invoice_number}`;
                    let pickUpCreditNoteUrl = `/admin/work-order/receipt/pick-up-credit-note/${this.dataForm?.invoice_number}`;
                    await this.fetchData(this.dataForm.receipt_from == 'invoice' ? pickUpInvoiceUrl : pickUpCreditNoteUrl,
                        async (res) => {
                            ...
                            let dataSet = {
                                ...
                                invoice_number: this.dataForm?.receipt_from == 'invoice' ? res?.data?.invoice_number : res?.data?.credit_note_number,
                            };
                            ...
                            this.$store.confirmDialog.open({
                                ...
                                afterClosed: (result) => {
                                    if (result) {
                                        ...
                                        setTimeout(() => {
                                            Axios({
                                                url: `{{ route('admin-work-order-receipt-save-new') }}`,
                                                method: 'POST',
                                                data: {
                                                    ...data
                                                }
                                                ...
                                                this.dialogClose();
                                                this.$store.create.options.afterClose(res);
