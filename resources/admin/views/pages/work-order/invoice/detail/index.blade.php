<template x-data="{}" x-if="$store.invoiceDetail.active">
    <div class="dialog" x-data="xInvoiceDetail" x-bind:style="{ zIndex: $store.libs.getLastIndex() + 1 }">
                    ...
                    <form id="form" class="form-wrapper" action="#" method="POST">
                        <template x-if="data">
                            <div class="form-body formBodyInvoiceDetail">
                                <?php $pageTable = 'admin::pages.work-order.invoice.detail.table'; ?>
        ...
        @include('admin::pages.work-order.invoice.detail.reason.add')
    </div>
</template>
...
<script>
    Alpine.data('xInvoiceDetail', () => ({
        ...
        async init() {
            ...
            let params =
                `order_id=${order?.id}&order_type_id=${order?.type_id}&btn_type=${btn_type}`;
            let url = `/admin/work-order/invoice/view-detail/${dataStore.id}?${params}`;
            let delayQuery = null;
        ...
        async excel() {
            let url = `/admin/work-order/invoice/export-invoice-excel/${this.data.id}`;
            this.submitLoading = true;
            setTimeout(async () => {
        ...
        excelHeaderTable(worksheet, data, dataHeaderNumberRow, style_font, align_left, style_border,
            ...
            let dataHeaderTable = [
                ...
                [
                    ...
                    `P.O. Nº  :  ${data?.order?.order_number}`
                ],
                [
                    "អ្នកទទួល/ Attention: ",
                    ""
        ...
        printPageArea(areaID) {
            ...
            document.title =
                `invoice_number_${this.data.invoice_number}_on_date_${date}`;
            printJS({
                printable: areaID,
                type: 'html',
                    ...
                    if (result) {
                        ...
                        dataTimeOut = setTimeout(() => {
                            $htmlGenerateToPdf(blockIdHtml, nameFile, (file) => {
                                ...
                                Axios.post(
                                    '/admin/work-order/invoice/dmc-submit',
                                    formData, {
                                        headers: {
            ...
            this.$store.confirmDialog.open({
                ...
                afterClosed: (result) => {
                    if (result) {
                        ...
                        clearTimeout(dataTimeOut);
                        dataTimeOut = setTimeout(() => {
                            Axios.post('/admin/work-order/invoice/dmc-submit',
                                formData, {
                                    headers: {
                                        'Content-Type': 'multipart/form-data',
                                    }
