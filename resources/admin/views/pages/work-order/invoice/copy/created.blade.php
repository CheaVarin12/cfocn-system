<template x-data="{}" x-if="$store.copyInvoiceCreate.active">
    <div class="dialog" x-data="xCopyInvoiceCreate" x-bind:style="{ zIndex: $store.libs.getLastIndex() + 1 }">
                            ...
                            <div class="row-3">
                                <div class="form-row">
                                    ...
                                    <input type="text" name="invoice_number" x-model="invoice_number"
                                        placeholder="Enter invoice number" :disabled="formDisable">
...
<script>
    Alpine.data('xCopyInvoiceCreate', () => ({
        submitLoading: false,
        sub_total: {
            dollar: 0,
            khmer: 0
        ...
        async init() {
            ...
            let dataStore = this.$store.copyInvoiceCreate.options.data;
        ...
        dialogClose() {
            this.$store.copyInvoiceCreate.active = false;
        },
...
<script>
    Alpine.store('copyInvoiceCreate', {
        active: false,
        options: {
            data: null,
            selected: null,
    ...
    window.copyInvoiceCreate = (options) => {
        Alpine.store('copyInvoiceCreate', {
            active: true,
            options: {
                ...Alpine.store('copyInvoiceCreate').options,
                ...options
            }
        });
    };
