<template x-data="{}" x-if="$store.copyInvoiceCreateSale.active">
    <div class="dialog" x-data="xCopyInvoiceCreateSale" x-bind:style="{ zIndex: $store.libs.getLastIndex() + 1 }">
                                                    ...
                                                    <div class="row table-row-8">
                                                        <span class="marginTop7">
                                                            <button type="button" class="delete"
                                                                @click="removeInput(index)" :disabled="formDisable">
...
<script>
    Alpine.data('xCopyInvoiceCreateSale', () => ({
        submitLoading: false,
        sub_total: {
            dollar: 0,
            khmer: 0
        ...
        exchang_rate: @json($rate) ? @json($rate).rate : "",
        total_qty: 0,
        invoice_number: null,
        note: null,
        remark: null,
        ...
        async init() {
            ...
            let dataStore = this.$store.copyInvoiceCreateSale.options.data;
        ...
        calcuatorAmount(type = null) {
            ...
            //khmer
            this.grand_total.khmer = this.numberRound(this.grand_total.dollar * Number(this.exchang_rate));
                            ...
                            this.$store.confirmDialog.open({
                                ...
                                afterClosed: (result) => {
                                    if (result) {
                                        ...
                                        setTimeout(() => {
                                            Axios({
                                                url: `{{ route('admin-work-order-invoice-copy-invoice') }}`,
                                                method: 'POST',
                                                data: {
                                                    ...data,
                                                    id: null,
        ...
        dialogClose() {
            this.$store.copyInvoiceCreateSale.active = false;
        },
...
<script>
    Alpine.store('copyInvoiceCreateSale', {
        active: false,
        options: {
            data: null,
            selected: null,
    ...
    window.copyInvoiceCreateSale = (options) => {
        Alpine.store('copyInvoiceCreateSale', {
            active: true,
            options: {
                ...Alpine.store('copyInvoiceCreateSale').options,
                ...options
            }
        });
    };
