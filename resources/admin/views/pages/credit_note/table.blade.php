            ...
            <div class="table-body">
                @foreach ($data as $index => $item)
                    <div class="column heightAuto">
                        ...
                        <div class="row table-row-15 text-start">
                            <span> {!! $item->purchase->project->name ? $item->purchase->project->name : '--' !!} </span>
                        </div>
                        <div class="row table-row-10">
                        ...
                        <div class="row table-row-5">
                            @canany(['credit-note-update', 'credit-note-detail', 'credit-note-dmc-submit'])
                                <div class="dropdown">
                                    <i data-feather="more-vertical" class="action-btn" id="dropdownMenuButton"
                                        data-mdb-toggle="dropdown" aria-expanded="false">
                                    </i>
                                    <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                        @can('credit-note-detail')
                                            <li>
                                        ...
                                        @if (checkValidate($item->issue_date))
                                            @if ($status != 3)
                                                @can('credit-note-update')
                                                    @if ($item->status == 2)
                                                        <li>
                                                            <a class="dropdown-item enable-btn"
                                                                onclick="$onConfirmMessage(
                                                        '{!! route('admin-credit-note-status', ['id' => $item->id, 'status' => 1]) !!}',
                                                        ...
                                                        @if ($item->doc_status != 'is_send')
                                                            ...
                                                            <li>
                                                                <a class="dropdown-item disable-btn"
                                                                    onclick="$onConfirmMessage(
                                                        '{!! route('admin-credit-note-status', ['id' => $item->id, 'status' => 2]) !!}',
