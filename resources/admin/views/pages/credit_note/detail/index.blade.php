                ...
                <div class="form-admin" id="my-section">
                    <form id="form" class="form-wrapper" action="#" method="POST">
                        <div class="form-body formBodyInvoiceDetail">
                            <?php $pageTable = 'admin::pages.credit_note.detail.table'; ?>
...
<script>
    Alpine.data('xInvoiceDetail', () => ({
        ...
        async init() {
            ...
            let url = `/admin/credit-note/view-detail/${dataStore.id}?${params}`;
            setTimeout(async () => {
                try {
                    await this.fetchData(url, (res) => {
                        ...
                        this.data.dataCustomer = this.data?.data_customer ? this
                            .jsonPase(this.data.data_customer) : null;
        ...
        excelTableZero(worksheet, data, Khmer_OS, style_border, align_center, align_left,
            ...
            data.credit_note_detail.forEach((item, index) => {
                worksheet.addRow([
                    "",
                    index + 1,
                    item?.service?.name,
        ...
        excelHeaderTable(worksheet, data, dataHeaderNumberRow, style_font, align_left, style_border,
            ...
            let dataHeaderTable = [
                [
                    ...
                    `លេខរៀងវិក្កយបត្រ/​ Invoice Nº​ : ${data?.credit_note?.credit_note_number}`
                ],
                [
                ...
                [
                    ...
                    `P.O. Nº  :  ${data?.invoices?.po_number ?? data?.purchase?.po_number}` 
                ],
                [
                    "អ្នកទទួល/ Attention : ",
        ...
        dmcSubmitOld() {
            this.$store.confirmDialog.open({
                ...
                afterClosed: (result) => {
                    if (result) {
                        this.submitLoading = true;
                        let nameFile = 'credit_note_' + moment().format(
                            'DD_MM_YYYY_H_mm_ss');
                        ...
                        dataTimeOut = setTimeout(() => {
                            $htmlGenerateToPdf(blockIdHtml, nameFile, (file) => {
                                ...
                                formData.append("invoice_id", this.data
                                    ?.credit_note?.id);
                                Axios.post('/admin/credit-note/dmc-submit',
                                    formData, {
                                        headers: {
