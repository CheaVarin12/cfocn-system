<template x-data="{}" x-if="$store.copyInvoiceMultiplePacSale.active">
    <div class="dialog" x-data="xCopyInvoiceMultiplePacSale" x-bind:style="{ zIndex: $store.libs.getLastIndex() + 1 }">
...
<script>
    Alpine.data('xCopyInvoiceMultiplePacSale', () => ({
        submitLoading: false,
        sub_total: {
            dollar: 0,
            khmer: 0
        ...
        async init() {
            ...
            let dataStore = this.$store.copyInvoiceMultiplePacSale.options.data;
      ...
      submitFrom() {
                ...
                if (valid.length > 0) {
                    ...
                    this.validationInstallNumber(res => {
                        if (res) {
                            ...
                            let dataStore = this.$store.copyInvoiceMultiplePacSale.options
                                .data;
        ...
        dialogClose() {
            this.$store.copyInvoiceMultiplePacSale.active = false;
        },
...
<script>
    Alpine.store('copyInvoiceMultiplePacSale', {
        active: false,
        options: {
            data: null,
            selected: null,
    ...
    window.copyInvoiceMultiplePacSale = (options) => {
        Alpine.store('copyInvoiceMultiplePacSale', {
            active: true,
            options: {
                ...Alpine.store('copyInvoiceMultiplePacSale').options,
                ...options
            }
        });
    };
