<template x-data="{}" x-if="$store.invoiceDetail.active">
    <div class="dialog" x-data="xInvoiceDetail" x-bind:style="{ zIndex: $store.libs.getLastIndex() + 1 }">
        ...
        <link rel="stylesheet" href="{{ asset('admin-public/css/invoice/detail.css') }}">
                ...
                <div class="form-admin" id="my-section">
                    <form id="form" class="form-wrapper" action="#" method="POST">
                        <div class="form-body formBodyInvoiceDetail">
                            <?php $pageTable = 'admin::pages.invoice.detail.table'; ?>
                            <template x-if="!data.purchase_type">
                                <div class="conditionComponent">
                                    <template x-if="data?.check_rate_first == 0 && data?.check_rate_seconde==0">
                                        @include($pageTable . '.zero')
                                    </template>
                                    <template x-if="data?.check_rate_first > 0 && data?.check_rate_seconde==0">
                                        @include($pageTable . '.one_rate')
                                    </template>
                                    <template
                                        x-if="(data?.check_rate_seconde > 0 && data?.check_rate_first==0) || (data?.check_rate_seconde > 0 && data?.check_rate_first > 0)">
                                        @include($pageTable . '.two_rate')
                                    </template>
                                </div>
                            </template>
                            <template x-if="data.purchase_type">
                                @include($pageTable . '.sale')
                            </template>
                        </div>
                        <template x-if="!$store.invoiceDetail?.options?.dmcBtn">
                        ...
                        <template x-if="$store.invoiceDetail?.options?.dmcBtn">
                            <div class="form-footer">
                                <template x-if="$store.invoiceDetail?.options?.typeBtnStatus == 'void'">
                                    <button type="button" class="yellow" @click="addReasonDialog()">
                                        <i class='bx bx-plus bx-tada-hover'></i>
                                        <span>Add Reason</span>
                                    </button>
                                </template>
                                {{-- x-bind:disabled="!connectonServer.status" --}}
                                {{-- <button type="button" class="primary" @click="dmcSubmit()">
                                    <i class='bx bx-send bx-tada-hover'></i>
                                    <span>Submit</span>
                                </button> --}}
                                <button type="button" class="primary" @click="dmcSubmitII()">
                                    <i class='bx bx-send bx-tada-hover'></i>
                                    <span>Submit</span>
                                </button>
                                <button type="button" class="close" @click="dialogClose()">
        ...
        @include('admin::pages.invoice.detail.reason.add')
    </div>
</template>
...
<script>
    Alpine.data('xInvoiceDetail', () => ({
        ...
        async init() {
            ...
            let params =
                `purchase_id=${purchase?.id}&purchase_type_id=${purchase?.type_id}&btn_type=${btn_type}`;
            let url = `/admin/invoice/view-detail/${dataStore.id}?${params}`;
            let delayQuery = null;
            ...
            delayQuery = setTimeout(async () => {
                try {
                    await this.fetchData(url, (res) => {
                        ...
                        this.data.dataCustomer = this.data?.data_customer ? this
                            .jsonPase(this.data.data_customer) : this.data
                            ?.customer;
        ...
        async excel() {
            let url = `/admin/invoice/export-invoice-excel/${this.data.id}`;
            this.submitLoading = true;
            setTimeout(async () => {
        ...
        async RunExcelJSExport(data) {
            ...
            if (!data?.purchase_type) {
                if (data.check_rate_first <= 0 && data.check_rate_seconde <= 0) {
                    i = this.excelTableZero(worksheet, data, Khmer_OS, style_border,
                        align_center, align_left, dataHeaderNumberRow, dataHeaderTableNumberRow,
                        dataColumn, Khmer_OS_Battambang, style_font);
                } else if (data.check_rate_first > 0 && data.check_rate_seconde <= 0) {
                    i = this.excelTableOneRate(worksheet, data, Khmer_OS, style_border, align_center,
                        align_left,
                        dataHeaderNumberRow,
                        ...
                        .check_rate_first <= 0 && data.check_rate_seconde > 0)) {
                    i = this.excelTableTwoRate(worksheet, data, Khmer_OS, style_border, align_center,
                        align_left,
                        dataHeaderNumberRow,
                        dataHeaderTableNumberRow,
                        dataColumn, Khmer_OS_Battambang, style_font);
                ...
                i = this.excelTableSale(worksheet, data, Khmer_OS, style_border, align_center,
                    align_left,
                    dataHeaderNumberRow,
                    dataHeaderTableNumberRow,
                    dataColumn, Khmer_OS_Battambang, style_font);
            ...
            const titleExportName = "invoice_" + this.dateFormatEn(moment(),
                'DD_MM_YYYY_H:mm:ss');
        ...
        //Zero
        excelTableZero(worksheet, data, Khmer_OS, style_border, align_center, align_left,
            dataHeaderNumberRow,
            dataHeaderTableNumberRow,
            dataColumn, Khmer_OS_Battambang, style_font) {
            // Add Header Row
        ...
        //RateOne
        excelTableOneRate(worksheet, data, Khmer_OS, style_border, align_center, align_left,
            dataHeaderNumberRow,
            dataHeaderTableNumberRow,
            dataColumn, Khmer_OS_Battambang, style_font) {
            // Add Header Row
        ...
        //RateTwo
        excelTableTwoRate(worksheet, data, Khmer_OS, style_border, align_center, align_left,
            dataHeaderNumberRow,
            dataHeaderTableNumberRow,
            dataColumn, Khmer_OS_Battambang, style_font) {
            // Add Header Row
        ...
        //Sale
        excelTableSale(worksheet, data, Khmer_OS, style_border, align_center, align_left,
            dataHeaderNumberRow,
            dataHeaderTableNumberRow,
            dataColumn, Khmer_OS_Battambang, style_font) {
            // Add Header Row
            ...
            data.invoice_detail.forEach((item, index) => {
                worksheet.addRow([
                    ...
                    item?.uom,
                    item?.price,
                    (data?.install_number ?? 0) + ' / ' + (data?.charge_number ?? 0),
                    item?.amount
                ]);
                //setStyle
                dataColumn.forEach((column) => {
        ...
        excelHeader(worksheet, data, align_center, lastColumn, style_font, Khmer_OS_Siemreap,
            ...
            let dataHeader = [
                ...
                `លេខអត្តសញ្ញាណកម្ម អតប (VATIN) : ${data?.purchase?.project?.vat_tin}`,
        ...
        excelHeaderTable(worksheet, data, dataHeaderNumberRow, style_font, align_left, style_border,
            ...
            let dataHeaderTable = [
                [
                    ...
                    `លេខរៀងវិក្កយបត្រ/​ Invoice Nº​ : ${data?.invoice_number}`
                ],
                [
        ...
        excelFooterTable(worksheet, data, dataColumn, Khmer_OS_Siemreap, style_font, align_left,
            ...
            if (!data?.purchase_type) {
                if (data.check_rate_first <= 0 && data.check_rate_seconde <= 0) {
                    this.footerTableZero(worksheet, dataTableFooterLeft, dataTableFooterRight, i,
                        style_font,
                        Khmer_OS_Siemreap,
                        align_left, dataColumn,
                        style_border, align_right, align_center, Calibri, columnHeaderTableMarge);
                } else if (data.check_rate_first > 0 && data.check_rate_seconde <= 0) {
                    this.footerTableOneRate(worksheet, dataTableFooterLeft, dataTableFooterRight, i,
                        style_font,
                        Khmer_OS_Siemreap,
                        align_left, dataColumn,
                        style_border, align_right, align_center, Calibri, columnHeaderTableMarge);
                        ...
                        .check_rate_first <= 0 && data.check_rate_seconde > 0)) {
                    this.footerTableTwoRate(worksheet, dataTableFooterLeft, dataTableFooterRight, i,
                        style_font,
                        Khmer_OS_Siemreap,
                        align_left, dataColumn,
                        style_border, align_right, align_center, Calibri, columnHeaderTableMarge);
                ...
                this.footerTableOneRate(worksheet, dataTableFooterLeft, dataTableFooterRight, i, style_font,
                    Khmer_OS_Siemreap,
                    align_left, dataColumn,
                    style_border, align_right, align_center, Calibri, columnHeaderTableMarge);
            }
        ...
        //ZeroWithSale
        footerTableZero(worksheet, dataTableFooterLeft, dataTableFooterRight, i, style_font,
            ...
            dataTableFooterLeft.forEach((item, index) => {
                ...
                worksheet.addRow([
                    ...
                    "",
                    Number(dataTableFooterRight[index].total[0]),
                    Number(dataTableFooterRight[index].total[1]),
                ]);
                ...
                dataColumn.forEach((column) => {
                    style_font.name = Khmer_OS_Siemreap;
                    if ((index == 0 || index == 1) && (column == "B" || column == "G" ||
                            column == "H")) {
        ...
        //FooterRateOne
        footerTableOneRate(worksheet, dataTableFooterLeft, dataTableFooterRight, i, style_font,
            Khmer_OS_Siemreap, align_left, dataColumn,
            style_border, align_right, align_center, Calibri, columnHeaderTableMarge) {
            let columnLeft = columnHeaderTableMarge[0] + i + ':' + columnHeaderTableMarge[1] +
                i;
            ...
            dataTableFooterLeft.forEach((item, index) => {
                ...
                worksheet.mergeCells(`B${indexData}:D${indexData}`);
                worksheet.mergeCells(`E${indexData}:G${indexData}`);
                worksheet.getRow(indexData).height = 28;

                //setStyle
                dataColumn.forEach((column) => {
                    style_font.name = Khmer_OS_Siemreap;
                    if ((index == 0 || index == 1) && (column == "B" || column == "H" ||
                            column == "I")) {
                        worksheet.getCell(column + indexData).font = {
                            ...style_font,
                            name: Calibri,
        ...
        //FooterRateTwo
        footerTableTwoRate(worksheet, dataTableFooterLeft, dataTableFooterRight, i, style_font,
            Khmer_OS_Siemreap, align_left, dataColumn,
            style_border, align_right, align_center, Calibri, columnHeaderTableMarge) {
            let columnLeft = columnHeaderTableMarge[0] + i + ':' + columnHeaderTableMarge[1] +
                i;
            ...
            dataTableFooterLeft.forEach((item, index) => {
                ...
                worksheet.mergeCells(`B${indexData}:D${indexData}`);
                worksheet.mergeCells(`E${indexData}:H${indexData}`);
                worksheet.getRow(indexData).height = 28;

                //setStyle
                dataColumn.forEach((column) => {
                    style_font.name = Khmer_OS_Siemreap;
                    if ((index == 0 || index == 1) && (column == "B" || column == "I" ||
                            column == "J")) {
                        worksheet.getCell(column + indexData).font = {
                            ...style_font,
                            name: Calibri,
        ...
        dmcSubmitOld() {
            let {
                typeBtnStatus
            } = this.$store.invoiceDetail.options;
            this.$store.confirmDialog.open({
                ...
                afterClosed: (result) => {
                    if (result) {
                        this.submitLoading = true;
                        let nameFile = 'invoice_number_' + moment().format(
                            'DD_MM_YYYY_H_mm_ss');
        ...
        addReasonDialog() {
            addReason({
                ...
                afterClose: (res) => {
                    this.des_reason = res.des_reason;
                }
            });
        }
    }));
