<template x-data="{}" x-if="$store.editInvoiceMultiplePacSale.active">
    <div class="dialog" x-data="xEditInvoiceMultiplePacSale" x-bind:style="{ zIndex: $store.libs.getLastIndex() + 1 }">
...
<script>
    Alpine.data('xEditInvoiceMultiplePacSale', () => ({
        submitLoading: false,
        sub_total: {
            dollar: 0,
            khmer: 0
        ...
        async init() {
            ...
            let dataStore = this.$store.editInvoiceMultiplePacSale.options.data;
                ...
                try {
                    await this.fetchData(url, (res) => {
                        ...
                        this.po_number = res?.invoice?.po_number ?? res?.invoice
                            ?.purchase?.po_number;
                        this.exchange_rate = res.invoice.exchange_rate ?? res.rate
                            ?.rate;
        ...
        calculatorAmount(type = null) {
            ...
            //khmer
            this.grand_total.khmer = this.numberRound(this.grand_total.dollar * Number(this.exchange_rate));
            if (this.tax_status != 2) {
      ...
      submitFrom() {
                ...
                if (valid.length > 0) {
                    ...
                    this.validationInstallNumber(res => {
                        if (res) {
                            ...
                            let dataStore = this.$store.editInvoiceMultiplePacSale.options
                                .data;
        ...
        dialogClose() {
            this.$store.editInvoiceMultiplePacSale.active = false;
        },
...
<script>
    Alpine.store('editInvoiceMultiplePacSale', {
        active: false,
        options: {
            data: null,
            selected: null,
    ...
    window.editInvoiceMultiplePacSale = (options) => {
        Alpine.store('editInvoiceMultiplePacSale', {
            active: true,
            options: {
                ...Alpine.store('editInvoiceMultiplePacSale').options,
                ...options
            }
        });
    };
