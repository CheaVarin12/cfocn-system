            ...
            <div class="table-body">
                @foreach ($data as $index => $item)
                    <div class="column heightAuto">
                        ...
                        <div class="row table-row-50">
                            <span>{!! $item->customer->name_en ?? $item->customer->name_kh !!}</span>
                        </div>
                        <div class="row table-row-40">
                            <span>{!! $item->status == 1 ? 'Enabled' : 'Disabled' !!}</span>
                        </div>
                        <div class="row table-row-5">
                            @canany(['fttx-customer-price-update'])
                                <div class="dropdown">
                                    ...
                                    <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                        @if ($status != 'trash')
                                            @can('fttx-customer-price-update')
                                                <li>
                                                    ...
                                                    <li>
                                                        <a class="dropdown-item enable-btn"
                                                            onclick="$onConfirmMessage(
                                                                '{!! route('admin-customer-price-status', ['id' => $item->id, 'status' => 1]) !!}',
                                                ...
                                                @else
                                                    <li>
                                                        <a class="dropdown-item disable-btn"
                                                            onclick="$onConfirmMessage(
                                                                '{!! route('admin-customer-price-status', ['id' => $item->id, 'status' => 2]) !!}',
