...
@section('layout')
    <div class="form-admin" x-data="xService">
        ...
        <form id="form" class="form-wrapper" action="{!! route('admin-service-save', $data->id) !!}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-body">
                    ...
                    <div class="form-row">
                        ...
                        <select name="type_id">
                            @foreach($types as $value)
                                <option value="{!!$value->id!!}" {!! $value->id == $data->type_id? 'selected' : '' !!}>{!!$value->name!!}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                <div class="row-2">
                    <div class="form-row">
                        ...
                        <input type="text" name="name" value="{{ $data->name ? $data->name : old('name') }}" placeholder="">
                        @error('name')
                ...
                <div class="row">
                    <div class="form-row">
                        ...
                        <textarea placeholder="" name="description" row="3">{!!$data->description!!}</textarea>
                        @error('description')
                            <label class="error">{{ $message }}</label>
                        @enderror
                    </div>
