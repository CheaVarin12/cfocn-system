                                ...
                                @foreach ($data as $index => $item)
                                    <tr @click="detailDialog({{ $item }})">
                                        <td class="row table-row-10">
                                            {{$index+1}}&nbsp;/{{ $item?->register_date ? date('d-M-Y', strtotime($item->register_date)) : '' }}
                                        </td>
                                        <td class="row table-row-5">{{ $item?->customer_code ?? '' }}</td>
                                        <td class="row table-row-10 textLeft">{{ $item?->name_en ?? '' }}</td>
                                        ...
                                        <td class="row table-row-10">{{ $item?->latestPurchase?->type?->name ?? '' }}
                                        </td>
                                        <td class="row table-row-10">{{ $item?->latestPurchase?->pac_type ?? '' }}
                                        </td>
                                        <td class="row table-row-10">{{ $item?->latestPurchase?->total_qty ?? '' }}
                                        </td>
                                        <td class="row table-row-5">{{ $item?->latestPurchase?->length ?? '' }}</td>
                                        <td class="row table-row-10">
                                            {{ $item->status == 1 ? 'Active' : 'Deactive' }}
                                        </td>
                                        <td class="row table-row-10">
                                            @if ($item->status == 2)
                                                {{ $item->in_active_date ?? \Carbon\Carbon::parse($item->updated_at)->format('d/m/Y') }}
                                            @endif
                                        </td>
                                    </tr>
                                @endforeach
